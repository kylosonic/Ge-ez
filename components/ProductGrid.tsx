              ...
              >
                <div className="aspect-square w-full overflow-hidden rounded-xl bg-stone-200 group-hover:opacity-90 transition-opacity relative">
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 h-10 w-10 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm hover:bg-white text-stone-900 transition-all z-20 cursor-pointer"
                    aria-label={wishlistItems.includes(product.id) ? "Remove from wishlist" : "Add to wishlist"}
                  >
                    <Heart size={20} fill={wishlistItems.includes(product.id) ? "currentColor" : "none"} className={wishlistItems.includes(product.id) ? "text-red-500" : "text-stone-600"} />
                  </button>
                </div>
